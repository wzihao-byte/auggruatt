...
def main() -> None:
    ...
    parser.add_argument("--run-dir", type=str, required=True, help="Path to results/protocol/<run_name>")
    ...
    for seed in _resolve_seed_list(cfg["seed_list"]):
        ...
        checkpoint_path = Path(
            metrics_payload.get(
                "checkpoint_path",
                str(seed_dir / f"best_model_{str(cfg['model_variant']).strip().lower()}.pt"),
            )
        )
    ...
    write_csv(run_dir / "attention_diagnostics_summary.csv", seed_rows, fieldnames=list(seed_rows[0].keys()))
    ...
    write_csv(run_dir / "attention_diagnostics_mean_std.csv", summary_rows, fieldnames=["metric", "mean", "std"])
